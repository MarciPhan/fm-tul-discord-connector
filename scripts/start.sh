...
if ! command -v go &> /dev/null; then
    echo -e "${RED}❌ Go není nainstalováno!${NC}"
    echo "   Nainstalujte Go z: https://go.dev/dl/"
    exit 1
fi
GO_VERSION=$(go version | awk '{print $3}')
echo -e "${GREEN}✅ Go nalezeno: ${GO_VERSION}${NC}"

# --- 2. Kontrola .env ---
if [ ! -f ".env" ]; then
    if [ -f ".env.example" ]; then
        echo -e "${YELLOW}⚠️  .env soubor nenalezen. Kopíruji z .env.example...${NC}"
        cp .env.example .env
        echo -e "${YELLOW}   Prosím upravte .env a vyplňte konfiguraci!${NC}"
        echo ""
    else
        echo -e "${RED}❌ .env ani .env.example nenalezeny!${NC}"
        exit 1
    fi
fi
echo -e "${GREEN}✅ Konfigurace .env nalezena${NC}"

# --- 3. Stažení závislostí ---
echo -e "${BLUE}📦 Stahuji závislosti...${NC}"
go mod tidy
echo -e "${GREEN}✅ Závislosti připraveny${NC}"

# --- 4. Kompilace ---
echo -e "${BLUE}🔨 Kompiluji ${BINARY_NAME}...${NC}"
go build -buildvcs=false -o "${BINARY_NAME}" ./cmd/bot/
echo -e "${GREEN}✅ Kompilace úspěšná: ./${BINARY_NAME}${NC}"
